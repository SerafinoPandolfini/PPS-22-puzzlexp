...
class PathExtractorSpec extends AnyFlatSpec:
  ...
  "from a cell it" should "be able to extract a path for its respective image" in {
    val cellsWithPaths: List[(Cell, String)] = List(
      (BasicCell(position1_1), "cell_BS_W_C"),
      (WallCell(position2_1), "cell_WL_C"),
      (HoleCell(position1_2), "cell_HL_W_D_C"),
      (CliffCell(position2_2, Item.Empty, Direction.Left), "cell_CL_LEFT_C"),
      (CoveredHoleCell(position3_2), "cell_CH_W_C_C"),
      (ButtonCell(position3_1, color = Color.Red), "cell_BT_W_RED_C"),
      (ButtonCell(position3_3, color = Color.Blue, pressableState = PressableState.Pressed), "cell_BT_W_BLUE_P_C")
    )
