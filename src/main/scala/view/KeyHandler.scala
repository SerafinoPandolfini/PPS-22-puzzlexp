...
object KeyHandler:
  private class KeyHandlerImpl() extends KeyHandler:
    ...
    val keyActions: List[KeyAction] = List(
      KeyAction(KeyEvent.VK_A, ImageManager.CharacterLeft.path, Direction.Left),
      KeyAction(KeyEvent.VK_D, ImageManager.CharacterRight.path, Direction.Right),
      KeyAction(KeyEvent.VK_W, ImageManager.CharacterUp.path, Direction.Up),
      KeyAction(KeyEvent.VK_S, ImageManager.CharacterDown.path, Direction.Down)
    )
